...
class SupabaseAuthService:
    ...
    def _http_request(self, method: str, path: str, *, access_token: str | None = None, json: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        headers = {"apikey": settings.supabase_auth_key, "Content-Type": "application/json"}
        if access_token:
        ...
        if response.status_code >= 400:
            try:
                ...
                payload = {}
            message = payload.get("msg") or payload.get("message") or payload.get("error_description") or "Supabase authentication request failed"
    ...
    def signup(self, email: str, password: str, full_name: str | None) -> dict[str, Any]:
        ...
        if full_name: body["data"] = {"full_name": full_name}
        return self._http_request("POST", "/signup", json=body)

    def login(self, email: str, password: str) -> dict[str, Any]:
        return self._http_request("POST", "/token?grant_type=password", json={"email": email, "password": password})

    def refresh(self, refresh_token: str) -> dict[str, Any]:
        return self._http_request("POST", "/token?grant_type=refresh_token", json={"refresh_token": refresh_token})
    ...
    def update_password(self, access_token: str, password: str) -> dict[str, Any]:
        return self._http_request("PUT", "/user", access_token=access_token, json={"password": password})
